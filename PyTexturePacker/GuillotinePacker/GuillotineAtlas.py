...
class GuillotineAtlas(AtlasInterface):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(GuillotineAtlas, self).__init__(*args, **kwargs)

        width, height = self.size
        self.rect_list = [Rect(0 + self.border_padding,
    ...
    def expand(self, method=EXPAND_SHORT_SIDE):
        ...
        if method == self.EXPAND_BOTH:
            new_size = (self.size[0] * 2, self.size[1] * 2)
        elif method == self.EXPAND_WIDTH:
            new_size = (self.size[0] * 2, self.size[1])
        elif method == self.EXPAND_HEIGHT:
            new_size = (self.size[0], self.size[1] * 2)
        elif method == self.EXPAND_SHORT_SIDE:
            if self.size[0] <= self.size[1]:
                new_size = (self.size[0] * 2, self.size[1])
            else:
                new_size = (self.size[0], self.size[1] * 2)
        elif method == self.EXPAND_LONG_SIDE:
            if self.size[0] >= self.size[1]:
                new_size = (self.size[0] * 2, self.size[1])
            else:
                new_size = (self.size[0], self.size[1] * 2)
        ...
        for rect in self.rect_list:
            if rect.right == old_size[0] - self.border_padding:
                rect.right = self.size[0] - self.border_padding
            if rect.bottom == old_size[1] - self.border_padding:
                rect.bottom = self.size[1] - self.border_padding
        ...
        self.rect_list = list(filter(self._max_rect_list_pruning, self.rect_list))
    ...
    def place_image_rect(self, rect_index, image_rect):
        rect = self.rect_list[rect_index]
        ...
        for i, rect in enumerate(self.rect_list):
            if fake_image_rect.is_overlaped(rect):
                _new_rect_list.extend(self.cut(rect, fake_image_rect))
            else:
                _rect_list.append(rect)

        self.rect_list = _new_rect_list
        self.rect_list = list(filter(self._rect_list_pruning, _new_rect_list))
        self.rect_list.extend(_rect_list)

        self.image_rect_list.append(image_rect)

    def _rect_list_pruning(self, t_rect):
        for rect in self.rect_list:
            if t_rect in rect and t_rect != rect:
                return False
